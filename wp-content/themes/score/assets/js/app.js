...
jQuery(document).ready(function ($) {
    ...
    $("label").click(function (e) {
        ...
        if (e.target.childNodes[1] && e.target.childNodes[1].type == "radio") {
            ...
            $(".form-contact label.checked").find("input").prop("checked", false).removeAttr("checked");
    ...
    $(wpcf7Elm).on("wpcf7invalid", function (event) {
        if (event.detail.apiResponse.status == "validation_failed") {
            ...
            var toast = '<div style="position:fixed;" role="alert" aria-live="assertive" aria-atomic="true" class="toast" data-delay="20000">' + '<div class="toast-header">' + '<svg class="bd-placeholder-img rounded mr-2" width="20" height="20" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice" focusable="false" role="img"><rect fill="#dc3545" width="100%" height="100%"></rect></svg>' + '<strong class="mr-auto">' + "Erros de validação" + "</strong>" + "<small>11 mins ago</small>" + '<button type="button" class="ml-2 mb-1 close" data-dismiss="toast" aria-label="Close">' + '<span aria-hidden="true">&times;</span>' + "</button>" + "</div>" + '<div class="toast-body">' + message + "</div>" + "</div>";
            ...
            for (var key in event.detail.apiResponse.invalidFields) {
                if (event.detail.apiResponse.invalidFields.hasOwnProperty(key)) {
                    var element = event.detail.apiResponse.invalidFields[key];
                    ...
                    }).addClass("red-tooltip").tooltip("toggle");
                }
            }

            setTimeout(function () {
                $(".red-tooltip").removeAttr("data-placement tabindex data-toggle title data-original-title").tooltip("hide");
            }, 4000);
    ...
    $(".corners").click(function () {
        if ($(".nav_mobile-menu").hasClass("menu-show")) {
            $(".nav_mobile-menu").css("transform", "translateX(calc(100vw - 15px))").removeClass("menu-show");
        } else {
            $(".nav_mobile-menu").css("transform", "translateX(calc(30px))").addClass("menu-show");
        }
    });
    ...
    function insertToSwiper(container, itens, arrows, options) {
        ...
        if (opt.navigation) {
            $(itens).parent().append("<!-- If we need navigation buttons -->" + '<div class="swiper-button-prev ' + prev.replace(".", "") + '"></div>' + '<div class="swiper-button-next ' + next.replace(".", "") + '"></div>');
        }

        $(itens).addClass("swiper-slide").wrapAll("<div class='swiper-wrapper'></div>");
